...
def wait_for_contract(
    ...
    link_index, link = iphone.wait_for(
        ...
        start,
        remaining_seconds(deadline, "a new physical BLE link"),
        IPHONE_FATAL_SIGNATURES,
    )
    ...
    _, rssi_line = iphone.wait_for(
        f"LINK_RSSI link_session={link_session} rssi=",
        link_index + 1,
        remaining_seconds(deadline, "a connected-link RSSI measurement"),
        session_abort_signatures,
    )
    rssi_match = re.search(rf"LINK_RSSI link_session={link_session} rssi=(-?\d+)", rssi_line)
    ...
    if observed_rssi < minimum_rssi:
        raise RuntimeError(
            f"link RSSI {observed_rssi} dBm is below the required {minimum_rssi} dBm; "
            "place the iPhone and ESP32 closer before diagnosing protocol recovery"
        )
    ...
    while len(health_lines) < minimum_health_frames:
        try:
            cursor, health = iphone.wait_for(
                "HEALTH_DECODED",
                cursor,
                remaining_seconds(deadline, "post-handshake health frames"),
                session_abort_signatures,
            )
        except TimeoutError as error:
            raise TimeoutError(
                f"received {len(health_lines)}/{minimum_health_frames} health frames "
                "inside the recovery budget"
            ) from error
        health_lines.append(health)
        cursor += 1
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=-80,
        help="Fail a cycle whose connected-link RSSI is below this dBm floor (default: -80)",
    )
    parser.add_argument(
        "--expected-firmware",
...
def main() -> int:
    ...
    try:
        ...
        uart_log.assert_absent(ESP_FATAL_SIGNATURES, 0)
        ...
        for cycle in range(1, args.cycles + 1):
            ...
            uart_log.assert_absent(ESP_FATAL_SIGNATURES, uart_start)
            results.append({"cycle": cycle, "fault": fault, "result": "PASS", **recovery})
        ...
        summary = {
            ...
            "required_health_frames": args.health_frames,
