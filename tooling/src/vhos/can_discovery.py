...
class PassiveCANRecord:
    ...
    @classmethod
    def from_document(
        cls, document: dict[str, Any], *, line_number: int
    ) -> PassiveCANRecord:
        location = f"line {line_number}"
        if set(document) != OBSERVATION_KEYS:
            raise CANDiscoveryError(f"{location}: passive CAN fields are invalid.")
        ...
        if document.get("contract_version") != OBSERVATION_VERSION:
            raise CANDiscoveryError(
                f"{location}: passive CAN contract version is unsupported."
            )
        ...
        if not listen_only:
            raise CANDiscoveryError(
                f"{location}: evidence does not retain listen-only proof."
            )
        ...
        if not isinstance(raw_data, list) or len(raw_data) != 8:
            raise CANDiscoveryError(
                f"{location}: data must contain exactly eight bounded bytes."
            )
        data = tuple(
...
def load_passive_can_ndjson(
    paths: Iterable[Path],
) -> tuple[list[PassiveCANRecord], list[dict[str, Any]]]:
    ...
    for path in files:
        ...
        if total_bytes > MAX_INPUT_BYTES:
            raise CANDiscoveryError(
                "Passive CAN input exceeds the aggregate byte limit."
            )
        raw = path.read_bytes()
        file_records = 0
        for line_number, line in enumerate(raw.splitlines(), start=1):
            ...
            if len(records) > MAX_RECORDS:
                raise CANDiscoveryError(
                    "Passive CAN input exceeds the analysis record limit."
                )
        sources.append(
            {
                "name": path.name,
...
def analyze_passive_can(
    ...
    *,
    ...
    if len(set(identities)) != len(identities):
        raise CANDiscoveryError(
            "Duplicate passive CAN observation identities were provided."
        )
    ...
    sessions = [
        _session_summary(key, values) for key, values in sorted(by_session.items())
    ]
    ...
    total_observed_intervals = sum(
        max(0, item["sequence_span"] - 1) for item in sessions
    )
...
def _session_summary(
    ...
    return {
        ...
        "retained_record_rate_fps": _round(
            len(records) / duration if duration > 0 else 0.0
        ),
        "sequence_coverage": _round(len(records) / span if span else 0.0, digits=6),
        "unique_identifiers": len(
            {(item.identifier, item.extended) for item in records}
        ),
    }
...
def _identifier_summary(
    ...
    for index in range(min_length):
        ...
        byte_ranges.append(
            {"byte": index, "minimum": min(values), "maximum": max(values)}
        )
    dynamic_positions = [
    ...
    word_values = [
        ((item.data[0] << 8) | item.data[1])
        for item in records
        if item.data_length >= 2
    ]
    checksum_checked = 0
    checksum_matches = 0
    for record in records:
    ...
    return {
        ...
        "payload_change_rate": _round(
            changes / transitions if transitions else 0.0, digits=6
        ),
        ...
        for right_key in identifier_keys[left_index + 1 :]:
            ...
            for session_records in by_session.values():
                left = [
                    (
                        item.monotonic_microseconds,
                        float((item.data[0] << 8) | item.data[1]),
                    )
                    for item in session_records
                    if (item.identifier, item.extended) == left_key
                    and item.data_length >= 2
                ]
                right = [
                    (
                        item.monotonic_microseconds,
                    ...
                    for item in session_records
                    if (item.identifier, item.extended) == right_key
                    and item.data_length >= 2
                ]
...
def _resolve_inputs(paths: Iterable[Path]) -> list[Path]:
    ...
    if len(resolved) > MAX_INPUT_FILES:
        raise CANDiscoveryError("Passive CAN input contains too many files.")
