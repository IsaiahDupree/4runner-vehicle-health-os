...
| [`toyota-can-cross-platform-lineage.v1.json`](../../vehicle-signal-packs/toyota-can-cross-platform-lineage.v1.json) | Source-by-source Toyota/Lexus family recurrence, conflicts, and applicability boundaries |
...
|---|---|---|---|
| `0x2C4`, bytes `0..1`, big-endian | Engine speed candidate at `raw × 0.78125 rpm` | 659 records; candidate range `0–4421.875 rpm` | High-priority cross-model candidate. Independent Camry and Lexus experiments identify the RPM-related field, and four pinned community artifacts converge on the scale; shared upstream lineage means those scale artifacts are not four independent proofs, and exact target proof is still missing. |
| `0x2C4`, byte `3` | Intake-air-temperature candidate | 659 records; raw byte `30–50` | Two related-platform artifacts supply conflicting temperature transforms. Compare the raw byte with J1979 PID `0x0F` or Techstream; do not display degrees yet. |
| `0x2D0`, bytes `0..1`, big-endian | Transmission turbine/input speed candidate at `raw × 0.390625 rpm` | 626 records; candidate range `0–4429.297 rpm` | NFI independently finds RPM-related data in `0x2D0`, but only the related FJ mapping supplies the turbine semantic and scale. CSU left it unknown. A drive/shift capture is required. |
| `0x2D0`, byte `2 & 0x7F` | Transmission selector-code candidate | Values `0–8`, mostly `8` | The FJ mapping puts a selector candidate here, while the NFI Lexus paper places current gear in its fifth byte. Do not map either field to P/R/N/D until a stationary marked experiment resolves the conflict. |
| `0x2C1`, byte `6` | Accelerator-pedal candidate at `raw × 0.5%` | 631 records; candidate range `0–47.5%`, mostly zero | Independent Camry and Lexus experiments report pedal behavior at this byte position; the percent scale remains cross-model. |
| `0x025`, candidate signed 12-bit field in bytes `0..1` | Steering-wheel-angle candidate | 667 records; raw signed counts `-12..39`; bytes `4`, `5`, and `6` also agree in every retained frame | The family is strongly recurrent, but an iQ definition uses signed 11-bit while Prius/later definitions use signed 12-bit; reviewed sources also conflict on scale and direction (`×1.5`, `×1.0`, and `×-0.087890625` degrees). The repeated bytes are a consistency clue, not yet named channels. **No degree value is accepted.** |
...
- [Netherlands Forensic Institute Lexus IS250 study](https://www.researchgate.net/publication/355445796_Evaluation_of_a_common_practice_-_to_calculate_speed_from_EDR-reported_RPM): synchronized physical references, DLC CAN logging, controlled driving, and EDR replay independently corroborate `0x2C4`, `0x2C1`, and `0x2D0` families while exposing a gear-field conflict with the FJ mapping.
...
- [Version-pinned comma.ai opendbc Toyota sources](https://github.com/commaai/opendbc/tree/b4ef5e1cf406ff143fa67bdbfb154739d43279c9/opendbc/dbc): demonstrates recurring `0x025` steering definitions while exposing an iQ signed-11-bit versus Prius/later signed-12-bit layout conflict and cross-model scale differences.
...
- Toyota's exact-model-year [2005 4Runner owner manual](https://assets.sia.toyota.com/publications/en/om-s/OM35860U/pdf/OM35860U.pdf), archived [General 2005 Features CAN section](https://www.scribd.com/document/391265361/51-Power-Steering), adjacent 2006 [4Runner GSIC manual archive](https://gitlab.com/ToyotaManuals/rm00t1u/-/tree/729e2480c90ffd3525595400ddd69024077f487b), and related [Land Cruiser/Prado New Features CAN page](https://toyotamanuals.gitlab.io/PZ471-Z00W0-CA/htmlweb/ncf/ncf276e/m_05_0242.pdf): establish configuration boundaries, exact/related 500 kbit/s topologies, and tester-visible channels, but not raw target payload definitions. The General Features source is explicitly marked as an unversioned third-party mirror.
